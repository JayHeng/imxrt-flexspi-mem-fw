...
#if !defined(SKIP_DCDC_ADJUSTMENT) || (!SKIP_DCDC_ADJUSTMENT) || defined(MULTICORE_APP)
...
#if !defined(SKIP_FBB_ENABLE) || (!SKIP_FBB_ENABLE) || defined(MULTICORE_APP)
...
#if (defined(BYPASS_LDO_LPSR) && BYPASS_LDO_LPSR) || defined(MULTICORE_APP)
    PMU_StaticEnableLpsrAnaLdoBypassMode(ANADIG_LDO_SNVS, true);
    PMU_StaticEnableLpsrDigLdoBypassMode(ANADIG_LDO_SNVS, true);
#endif

#if (!defined(SKIP_LDO_ADJUSTMENT) || (!SKIP_LDO_ADJUSTMENT)) && !defined(MULTICORE_APP)
...
#if (__CORTEX_M == 7) || defined(MULTICORE_APP)
...
#if (__CORTEX_M == 4) || defined(MULTICORE_APP)
