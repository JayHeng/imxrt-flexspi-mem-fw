...
// this code.
//*****************************************************************************
#if defined (__MULTICORE_MASTER_SLAVE_M33SLAVE) || \
...
void boot_multicore_slave(void) {
    ...
    /* When kicking off the CM33 core from the CM7 core */
    #if defined(__MULTICORE_MASTER_SLAVE_M33SLAVE)
    	bootAddress = (uint32_t)(char *)&__core_m33slave_START__;
        IOMUXC_LPSR_GPR->GPR0 = IOMUXC_LPSR_GPR_GPR0_CM33_INIT_VTOR_LOW(bootAddress >> 3u);
        IOMUXC_LPSR_GPR->GPR1 = IOMUXC_LPSR_GPR_GPR1_CM33_INIT_VTOR_HIGH(bootAddress >> 16u);

        SRC->CTRL_M33CORE = SRC_CTRL_M33CORE_SW_RESET_MASK;
        SRC->SCR |= SRC_SCR_BT_RELEASE_M33_MASK;
    #endif
    /* When kicking off the CM7 core from the CM33 core */
...
#endif // defined (__MULTICORE_MASTER_SLAVE_M33SLAVE) ||
