...
void EDMA_ConfigChannelSoftwareTCD(edma_tcd_t *tcd, const edma_transfer_config_t *transfer)
{
    ...
    tcd->CSR |= (tcd->CSR & (~(uint16_t)(DMA_CSR_DREQ_MASK))) |
                DMA_CSR_DREQ(transfer->disableRequestAfterMajorLoopComplete) |
    ...
    if (transfer->enableChannelMinorLoopLink)
    {
        ...
        tcd->BITER |= DMA4_BITER_ELINKYES_ELINK_MASK;
        /* Set linked channel */
...
#if defined FSL_FEATURE_SOC_DMA4_COUNT && FSL_FEATURE_SOC_DMA4_COUNT
...
#else
    handle->channelBase->CH_CSR = handle->channelBase->CH_CSR & (~(DMA4_CH_CSR_DONE_MASK | DMA4_CH_CSR_ERQ_MASK));
#endif
}
...
{
#if FSL_EDMA_SOC_IP_EDMA
    if ((s_EDMAHandle[instance][channel]->base->INT >> channel) != 0U)
    {
        EDMA_HandleIRQ(s_EDMAHandle[instance][channel]);
        s_EDMAHandle[instance][channel]->base->CINT = channel;
    }
#else
    if ((s_EDMAHandle[instance][channel]->channelBase->CH_INT & DMA_CH_INT_INT_MASK) != 0U)
    {
        EDMA_HandleIRQ(s_EDMAHandle[instance][channel]);
        s_EDMAHandle[instance][channel]->channelBase->CH_INT |= DMA_CH_INT_INT_MASK;
    }
#endif
