...
#define FSL_NETC_DRIVER_VERSION (MAKE_VERSION(2, 0, 0))
...
enum _netc_status
{
    kStatus_NETC_Success,        /*!< Success */
    kStatus_NETC_Busy,           /*!< Busy */
...
typedef enum _netc_ep_tx_status
{
    ...
    kNETC_EPTxProgramErr =
        0x1, /*!< An error exists in either the Tx BD, the Tx ring registers, or combination of these. */
...
/*! @brief Defines the NETC Rx memory buffer alloc function pointer. */
typedef void *(*netc_rx_alloc_cb_t)(uint32_t length);

/*! @brief Defines the NETC Rx memory buffer free function pointer. */
typedef void (*netc_rx_free_cb_t)(void *address);
...
{
    netc_buffer_struct_t *buffArray; /*!< Buffer array. */
    uint16_t length;                 /*!< Buffer array length. */
} netc_frame_struct_t;
...
{
    bool isTsAvail;             /*!< Rx frame timestamp is available or not. */
    uint32_t timestamp;         /*!< The timestamp of this Tx frame. */
...
{
    kNETC_OuterStanCvlan   = 0x1U, /*!< 0x8100. */
    kNETC_OuterStanSvlan   = 0x2U, /*!< 0x88A8. */
    kNETC_OuterCustomVlan1 = 0x4U,
    kNETC_OuterCustomVlan2 = 0x8U,
    kNETC_InnerStanCvlan   = 0x1U, /*!< 0x8100. */
    kNETC_InnerStanSvlan   = 0x2U, /*!< 0x88A8. */
...
typedef struct _netc_port_parser_config
{
    ...
    bool enableL3Parser;      /*!< Enable/Disable parser for L3 */
    uint8_t l3PayloadCount;   /*!< L3 payload fields size in bytes */
    bool enableL4Parser;      /*!< Enable/Disable parser for L3 */
    uint8_t l4PayloadCount;   /*!< L3 payload fields size in bytes */
} netc_port_parser_config_t;

/*! @brief Port time gate config */
typedef struct _netc_port_tg_config
{
    bool enableTg;      /*!< Port time gating enable */
...
/*! @brief Port stream gate config */
typedef struct _netc_port_sg_config
{
    uint32_t pDelay : 24; /*!< Hold-Skew in ns, not effective on ports connected to a pseudo-MAC */
    uint32_t : 7;
    uint32_t ogcMode; /*!< Stream Gate Open Gate Check mode, 0b is check whether SFD is within the open gate interval,
                         1b is check whether the entire frame is within the open gate interval */
} netc_port_sg_config_t;

/*! @brief Port Tx/Rx SDU overhead config */
typedef struct _netc_port_sdu_config
{
...
/*! @brief Port TSD config */
typedef struct _netc_tsd_config
{
    uint32_t placeholder; /*!< Placeholder for compile */
} netc_tsd_config_t;

/*! @brief Port vlan accepted tag type enumerator */
typedef enum _netc_vlan_tag
{
    kNETC_VlanTagUntagged = 0x1,
    kNETC_VlanPromiscuous = 0x1 << 1U,
} netc_vlan_tag_t;

/*! @brief Accepted vlan tpid */
typedef struct _netc_port_vlan_config
{
    uint8_t innerTpidAllowedMask; /*!< PORT PTAR */
    uint8_t outterTpicAllowedMask;
} netc_port_vlan_config_t;
 ...
 */
typedef struct _port_cbs_config
{
...
typedef struct _port_sdu_config
{
...
/*! @brief Configuration for the Traffic Class */
typedef struct _netc_tx_tc_config
{
    bool enPreemption; /*!< Frames from traffic class are transmitted on the preemptable MAC, not supported on internal
                          port (ENETC 1 port and Switch port 4)*/
    bool enableCbs;    /*!< Enable Credit based shaper for traffic class  */
    port_sdu_config_t sduCfg;
    port_cbs_config_t cbsCfg; /*!< Configure transmit traffic class credit based shaper (PTC0CBSR0/PTC0CBSR1) if
...
/*! @brief Vlan classification config */
...
/*! @brief Port Vlan classification port config */
typedef struct _netc_vlan_classify_port_config
{
...
/*! @brief Qos Classification profile file (vlan PCP/DEI to IPV/DR map) */
...
/*! @brief Port Ingress Filter Port Config */
typedef struct _netc_ipf_port_config_t
{
    bool enL2Dos;
    bool enL3Dos;
    bool enIPFTable;
} netc_ipf_port_config_t;
 ...
 */
typedef struct _netc_psfp_isi_port_config
{
...
} netc_psfp_isi_port_config_t;

/*! @brief Port Ingress Congestion Management config */
...
{
    bool enableRevMii;                /*!< Enable RevMII mode. */
    netc_port_ts_select_t txTsSelect; /*!< Tx timestamp clock source. */
    netc_hw_mii_mode_t miiMode;       /*!< MII mode. */
    netc_hw_mii_speed_t miiSpeed;     /*!< MII Speed. */
    netc_hw_mii_duplex_t miiDuplex;   /*!< MII duplex. */
    bool enMergeVerify; /*!< Enable verify the merged preemption frame, need to enable when preemptMode is not zero */
    uint8_t
        mergeVerifyTime; /*!< The nominal wait time between verification attempts in milliseconds, range in 1 ~ 128 */
    netc_hw_preemption_mode_t preemptMode; /*!< When set to not zero, PMAC frames may be preempted by EMAC frames */
} netc_port_ethmac_t;
...
typedef struct _netc_port_common
{
    ...
    uint32_t pSpeed; /*!< Transmit Port Speed = 10Mbps * (pSpeed+1), Used by ETS, Qbu and to determine if cut-through is
                        permissable  */
    bool stompFcs;
    netc_port_sg_config_t rxSgCfg;    /*!< Port rx stream gate config */
    netc_port_ts_select_t rxTsSelect; /*!< Eth MAC Rx or pseudo MAC Tx timestamp clock source */
    netc_port_sdu_config_t sduCfg;
...
typedef struct _netc_port_phy_mac_discard_statistic
{
    ...
    uint64_t rxErrorFCS;
    uint64_t
        rxFragment; /*!< Count of MAC frames which is shorter than the MIN length and received with a wrong FCS/CRC. */
    uint64_t
        rxJabber; /*!< Count of MAC frames which is larger than the MAX length and received with a wrong FCS/CRC. */
    uint64_t rxDiscard;
...
{
    uint32_t
        rxReassembledFrame; /*!< Count of MAC frames that were successfully reassembled and delivered to the MAC. */
...
{
    uint32_t totalOctet;
    uint32_t unicastFrame;
    uint32_t multicastFrame;
    uint32_t boradcastFrame;
...
typedef enum _netc_tb_cmd
{
    ...
    kNETC_AddQueryAndUpdateEntry = 0xEU
} netc_tb_cmd_t;
...
typedef enum _netc_cmd_generic_error
{
    kNETC_NoError              = 0x0U,
...
{
    kNETC_ISNotPreformFRER = 0, /*!< Not perform Frame FRER sequence generation function */
    kNETC_ISPreformFRER         /*!< Perform Frame FRER sequence generation function */
...
typedef enum _netc_tb_is_forward_action
{
    ...
    kNETC_ISRedirectToMgmtPort, /*!< Frame be Re-direct frame to switch management port, Switch only */
    kNETC_ISAllow = 1U,         /*!< Frame is allow without setting the pre L2 filtering SI bitmap, ENETC only */
    kNETC_ISAllowWithSIMap,     /*!< Frame is allow with setting the pre L2 filtering SI bitmap to the value configured
...
#endif
/*! @brief Stream Gate Control List Table Response Data Buffer MAX length = 16bytes + 256 8bytes gating entries */
#define NETC_TB_SGCL_MAX_RES_BUFFER_LENGTH (16U + 256U * 8U)

/*! @brief Stream Gate Control List Table Entry config length, x is the number of stream gate list entries */
#define NETC_TB_SGCL_ENTRY_CONFIG_LENGTH(x) (12U + 8U * (x))

/*! @brief Stream Gate Control List Table add/QUERY Entry Data Buffer length, x is the number of stream gate list
 * entries */
#define NETC_TB_SGCL_ADD_OR_QUERY_BUFFER_LENGTH(x) (16U + 8U * (x))
...
typedef struct _netc_tb_sgcl_cfge
{
    ...
    struct
    {
        ...
        uint32_t
            iomen : 1;     /*!< Interval Octet Maximum Enabled for Gate Entry, 0b = Don'track count, 1b = Track count */
...
/*! @brief Stream Gate Control List table entry config */
typedef struct _netc_tb_sgcl_config
{
    uint32_t entryID;
    netc_tb_sgcl_cfge_t cfge;
} netc_tb_sgcl_config_t;
...
{
    uint32_t res;   /*!< Hold for request->entryID */
    uint8_t cfge[]; /*!< Configuration Element Data size is variable */
...
typedef struct _etc_tb_fdb_search_criteria
{
    ...
    struct
    {
        ...
        uint8_t : 6;
        etc_tb_fdb_sc_cfge_mc_t cfgeMc : 2; /*!< Configuration Element data match criteria */
        uint8_t : 6;
...
typedef netc_tb_fdb_search_criteria_t netc_tb_l2mcf_search_criteria_t;
...
#define NETC_TB_TGS_MAX_ENTRY (0x100)
...
{
    uint8_t bandWeight : 4;
    netc_hw_enetc_si_vlan_type vlanCtrl : 4; /*!< VLAN Ethertypes can be inserted by the SI driver */
    uint8_t antiSpoofEnable : 1;
    uint8_t vlanInsertEnable : 1;
    uint8_t vlanExtractEnable : 1;
    uint8_t sourcePruneEnable : 1;
    uint8_t txRingUse; /*!< Number of TX Rings to be used */
    uint8_t rxRingUse; /*!< Number of RX Rings to be used */
} netc_hw_enetc_si_config_t;

/*! @brief Host Transfer Agent HTA configuration for RX */
typedef struct _netc_si_hta_config
{
    bool enRSS;
} netc_si_hta_config_t;
...
typedef union _netc_tx_bd
{
    struct
    {
        ...
        uint32_t isExtended : 1;      /*!< Is extended BD format. */
        uint32_t isFinal : 1;         /*!< Is final BD in the chain. */
    } standard;
    struct
    {
    ...
    struct
    {
        ...
        uint32_t txtsid : 16; /*!/ Transmit timestamp identifier. */
        uint32_t : 16;
        uint32_t : 32;
        uint32_t : 16;
...
typedef union _netc_rx_bd
{
    ...
    struct
    {
        ...
        uint8_t error;     /*!< Rx error. */
        uint8_t : 6;
        uint8_t isReady : 1; /*!< Is the Rx BD received. */
        uint8_t isFinal : 1; /*!< Is this the final BD. */
    } writeback;
    struct
    {
    ...
    struct
    {
        ...
        uint32_t error : 8;
        uint32_t : 6;
        uint32_t isReady : 1; /*!< Is the Rx BD received. */
        uint32_t isFinal : 1; /*!< Is this the final BD. */
    } resp;
} netc_rx_bd_t;
...
{
    uint16_t len;                     /*!< Size of BD ring which shall be multiple of 8 BD */
    ...
    uint8_t msixEntryIdx;    /*!< Tx ring interrupt entry index in MSIX table */
    // Configure
    bool isVlanInsert; /*!< Whether to support VLAN insert offload */
    bool isUserCRC;    /*!< Whether user provided the CRC32 - FCS at end of frame */
...
typedef struct _netc_tx_bdr
{
    ...
    uint16_t len;                    /*!< Length of this BD ring */
} netc_tx_bdr_t;
...
typedef struct _netc_rx_bdr_config
{
    ...
    bool extendDescEn;     /*!< False - Use 16B standard BD. True - Use 32B extended BD. */
    uint16_t len;          /*!< Length of BD ring, shall be multiple of 8/16 standard/extened BD. */
    netc_rx_bd_t *bdArray; /*!< BDR base address which shall be 128 bytes aligned */
    // Interrupt
    bool enThresIntr;        /*!< Whether to enable threshold interrupt. */
    bool enCoalIntr;         /*!< Enable/Disable interrupt coalescing. */
    uint32_t intrThreshold;  /*!< Interrupt coalescing packet threshold. */
    uint32_t intrTimerThres; /*!< Interrupt coalescing timer threshold, specified in NETC clock cycles. */
    uint8_t msixEntryIdx;    /*!< Rx ring interrupt entry index in MSIX table */
    // Configuration
...
{
    kNETC_DisableMACLearn      = 1U, /*!< No FDB lookup is performed, the frame is flooded. */
    kNETC_HardwareMACLearn     = 2U, /*!< FDB lookup is performed, and if there is no match, the frame is flooded. */
    kNETC_SeSoftwareMACLearn   = 3U, /*!< FDB lookup is performed, and if there is no match, the frame is discarded. */
    kNETC_UnseSoftwareMACLearn = 4U, /*!< FDB lookup is performed, and if there is no match, the frame is discarded. */
    kNETC_DisableMACLearnWithSMAC =
        5U /*!< FDB lookup is performed, and if there is no match, the frame is discarded. */
...
typedef struct _netc_swt_default_vlan_filter
{
    ...
    uint8_t stgID;          /*!< Spanning Tree Group Member ID, range in  */
...
/*! @brief Bridge port discard statistic */
typedef struct _netc_switch_port_discard_statistic
{
    uint32_t placeHolder;
} netc_switch_port_discard_statistic_t;
