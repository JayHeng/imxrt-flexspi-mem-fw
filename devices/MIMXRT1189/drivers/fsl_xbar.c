...
#define XBAR_INST_FROM_INPUT(input)   ((uint16_t)(input) >> 12U)
#define XBAR_INST_FROM_OUTPUT(output) ((uint16_t)(output) >> 12U)
...
static status_t XBAR_GetCtrlRegAndShift(xbar_output_signal_t output, volatile uint16_t **ctrlRegAddr, uint8_t *shift)
{
    ...
    if ((inst > ARRAY_SIZE(s_xbarInfo)) || (outputIndex > (s_xbarInfo[inst].regCtrlNum * 2U)))
    {
        status = kStatus_InvalidArgument;
    }
    else
    {
        *ctrlRegAddr = s_xbarInfo[inst].baseAddr + ((s_xbarInfo[inst].regCtrlOffset + outputIndex) / 2U);
...
status_t XBAR_SetSignalsConnection(xbar_input_signal_t input, xbar_output_signal_t output)
{
    ...
    if ((inst > ARRAY_SIZE(s_xbarInfo)) || (outputIndex > (s_xbarInfo[inst].regSelNum * 2U)))
    {
        status = kStatus_InvalidArgument;
    }
    else
    {
        selRegAddr  = s_xbarInfo[inst].baseAddr + ((s_xbarInfo[inst].regSelOffset + outputIndex) / 2U);
...
status_t XBAR_SetOutputSignalConfig(xbar_output_signal_t output, const xbar_control_config_t *controlConfig)
{
    ...
    if (status == kStatus_Success)
    {
        *ctrlRegAddr = ((controlConfig->activeEdge) << shiftInReg) | ((controlConfig->requestType) << shiftInReg);
    }
