...
void NETC_RxQosCInit(NETC_SW_ENETC_Type *base, const netc_qos_classify_profile_t *profile, bool enProfile1)
{
    ...
    {
        ipv[0] |= (profile->ipv[i] & NETC_SW_ENETC_VLANIPVMPR0_PCP_DEI_0_MASK) << (4U * i);
        ipv[1] |= (profile->ipv[i + 8] & NETC_SW_ENETC_VLANIPVMPR0_PCP_DEI_0_MASK) << (4U * i);
    }
    for (uint32_t i = 0U; i < 16; i++)
    {
        dr |= (profile->dr[i] & NETC_SW_ENETC_VLANDRMPR_PCP_DEI_0_MASK) << (2U * i);
    }
...
status_t NETC_CmdBDRInit(netc_cbdr_hw_t *base, const netc_cmd_bdr_config_t *config)
{
    ...
    else
    {
        ...
        base->CBDRPIR  = 0U;
        base->CBDRCIR  = 0U;
        base->CBDRLENR = config->bdLength & ENETC_SI_SICBDRLENR_LENGTH_MASK;
...
static void EP_CleanUsedBD(netc_cbdr_hw_t *base, netc_cmd_bdr_t *cbdr)
{
    ...
    {
        memset((void *)&cbdr->bdBase[curCleanIndex], 0, sizeof(netc_cmd_bd_t));
                               ...
                               netc_cbd_version_t version)
{
    ...
    if (kStatus_Success == status)
    {
...
#if defined(FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET) && FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET
        cbdr->bdBase[curIndex].req.addr =
            (uintptr_t)MEMORY_ConvertMemoryMapAddress((uint32_t)cbd->req.addr, kMEMORY_Local2DMA);
#endif
        ...
        base->CBDRPIR = (curIndex + 1U) % cbdr->bdLength;
...
status_t NETC_AddIPFTableEntry(netc_cbdr_handle_t *handle, netc_tb_ipf_config_t *config, uint32_t *entryID)
{
    ...
    status_t status     = kStatus_Success;
    memset(handle->buffer, 0, sizeof(netc_tb_ipf_req_data_t));
    ...
    handle->buffer->ipf.request.commonHeader.updateActions = kNETC_IPFStsEUpdate | kNETC_IPFCfgEUpdate;
...
status_t NETC_ResetIPFMatchCounter(netc_cbdr_handle_t *handle, uint32_t entryID)
{
    ...
    memset(handle->buffer, 0, sizeof(netc_tb_ipf_req_data_t));
    handle->buffer->ipf.request.entryID                    = entryID;
    handle->buffer->ipf.request.commonHeader.updateActions = kNETC_IPFStsEUpdate;
    handle->buffer->ipf.request.commonHeader.queryActions  = 0U;
    cmdBd.req.addr                                         = (uintptr_t)handle->buffer;
    cmdBd.req.reqLength                                    = sizeof(netc_tb_ipf_req_data_t);
    cmdBd.req.resLength                                    = 4U;
...
status_t NETC_AddISITableEntry(netc_cbdr_handle_t *handle, netc_tb_isi_config_t *config, uint32_t *entryID)
{
    netc_cmd_bd_t cmdBd = {0};
    status_t status     = kStatus_Success;
    memset(handle->buffer, 0, sizeof(netc_tb_isi_req_data_t));
...
{
    netc_cmd_bd_t cmdBd = {0};
    memset(handle->buffer, 0, sizeof(netc_tb_is_req_data_t));
...
{
    netc_cmd_bd_t cmdBd = {0};
    memset(handle->buffer, 0, sizeof(netc_tb_isf_req_data_t));
...
status_t NETC_GetISCStatistic(netc_cbdr_handle_t *handle, uint32_t entryID, netc_tb_isc_stse_t *statistic)
{
    netc_cmd_bd_t cmdBd = {0};
    status_t status     = kStatus_Success;
    memset(handle->buffer, 0, sizeof(netc_tb_isc_req_data_t));
    handle->buffer->isc.request.entryID                    = entryID;
    handle->buffer->isc.request.commonHeader.updateActions = 0U;
    handle->buffer->isc.request.commonHeader.queryActions  = 0U;
    cmdBd.req.addr                                         = (uintptr_t)handle->buffer;
...
{
    netc_cmd_bd_t cmdBd = {0};
    memset(handle->buffer, 0, sizeof(netc_tb_sgi_req_data_t));
    ...
    handle->buffer->sgi.request.commonHeader.updateActions =
        kNETC_SGIAcfEUpdate | kNETC_SGICfgEUpdate | kNETC_SGISgisEUpdate;
...
status_t NETC_GetSGIState(netc_cbdr_handle_t *handle, uint32_t entryID, netc_tb_sgi_sgise_t *state)
{
    netc_cmd_bd_t cmdBd = {0};
    status_t status     = kStatus_Success;
    memset(handle->buffer, 0, sizeof(netc_tb_sgi_rsp_data_t));
    handle->buffer->sgi.request.entryID                    = entryID;
    handle->buffer->sgi.request.commonHeader.updateActions = 0U;
    handle->buffer->sgi.request.commonHeader.queryActions  = 0U;
    cmdBd.req.addr                                         = (uintptr_t)handle->buffer;
    ...
    {
        /* Workwround*/
        memcpy(&state->operCycleTimeExt, &handle->buffer->sgi.response.entryID, 5U);
        //        *state = handle->buffer->sgi.response.sgise;
    }
    return status;
}
...
status_t NETC_AddSGCLTableEntry(netc_cbdr_handle_t *handle, netc_tb_sgcl_config_t *config)
{
    netc_cmd_bd_t cmdBd = {0};
    uint16_t length;
    if ((((uintptr_t)handle->buffer % 16U) != 0U) || (config->cfge.listLength == 0U))
    {
        return kStatus_InvalidArgument;
    }
    length = config->cfge.listLength;
    /* The entry numbers is LIST_LENGTH -1 */
    config->cfge.listLength = length - 1;
    memset(handle->buffer, 0, sizeof(netc_tb_sgcl_req_data_t));
    ...
    memcpy(&handle->buffer->sgcl.request.cfge, &config->cfge, (16U + length * 8U));
    cmdBd.req.addr       = (uintptr_t)handle->buffer;
    cmdBd.req.reqLength  = 24U + length * 8U;
    cmdBd.req.resLength  = 0U;
...
status_t NETC_GetSGCLState(netc_cbdr_handle_t *handle, uint32_t entryID, netc_tb_sgcl_sgclse_t *state)
{
    status_t status     = kStatus_Success;
    netc_cmd_bd_t cmdBd = {0};
    if (((uintptr_t)handle->buffer % 16U) != 0U)
    {
        return kStatus_Fail;
    }
    memset(handle->buffer, 0, sizeof(netc_tb_sgcl_req_data_t));
    ...
    cmdBd.req.addr                                          = (uintptr_t)handle->buffer;
    cmdBd.req.reqLength                                     = 8U;
    cmdBd.req.resLength                                     = NETC_TB_SGCL_MAX_RES_BUFFER_LENGTH;
    cmdBd.req.tableId                                       = kNETC_SGCLTable;
    cmdBd.req.cmd                                           = kNETC_QueryEntry;
    cmdBd.req.accessType                                    = kNETC_EntryIDMatch;
    status = NETC_CmdBDSendCommand(handle->base, handle->cmdr, &cmdBd, kNETC_NtmpV2_0);
...
{
    netc_cmd_bd_t cmdBd = {0};
    memset(handle->buffer, 0, sizeof(netc_tb_rp_req_data_t));
    ...
    handle->buffer->rp.request.commonHeader.updateActions = kNETC_RPCfgEUpdate | kNETC_RPFeEUpdate;
...
status_t NETC_ConfigTGSAdminList(netc_cbdr_handle_t *handle, netc_tb_tgs_gcl_t *config)
{
    ...
    /* Restore the administrative gate control list length */
    handle->buffer->tgs.request.entryID                    = config->entryID;
    handle->buffer->tgs.request.commonHeader.updateActions = 1U;
    handle->buffer->tgs.request.commonHeader.queryActions  = 0U;
    /* Initialize port Time Gate Scheduling entry administrative gate control list */
    memcpy(&handle->buffer->tgs.request.cfge, &config->baseTime, 20);
    memcpy(&handle->buffer->tgs.request.cfge.adminGcl[0], config->gcList, config->numEntries * 8U);
    cmdBd.req.addr       = (uintptr_t)handle->buffer;
...
status_t NETC_GetTGSOperationList(netc_cbdr_handle_t *handle, netc_tb_tgs_gcl_t *gcl, uint32_t length)
{
    ...
    memset(handle->buffer, 0, sizeof(netc_tb_tgs_data_t));
    ...
    if (kStatus_Success == status)
    {
        ...
        resOlseBase = (netc_tb_tgs_olse_t *)&((uint8_t *)handle->buffer)[36U + 8U * adminLen];
        ...
        else
        {
            ...
            memcpy(gcl->gcList, &resOlseBase->operGcl[0], (gcl->numEntries * 8U));
        }
    }
    return status;
}
