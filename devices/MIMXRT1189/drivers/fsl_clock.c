...
void CLOCK_InitSysPll2(const clock_sys_pll2_config_t *config)
{
    ...
    if ((ANADIG_PLL->PLL_528_CTRL & ANADIG_PLL_PLL_528_CTRL_POWERUP_MASK) != 0UL)
    {
        if ((config == NULL) ||
            ((0UL == (ANADIG_PLL->PLL_528_SS & ANADIG_PLL_PLL_528_SS_ENABLE_MASK)) && (!config->ssEnable)) ||
            ((ANADIG_PLL_PLL_528_SS_ENABLE(config->ssEnable) | ANADIG_PLL_PLL_528_SS_STOP(config->ss->stop) |
              ANADIG_PLL_PLL_528_SS_STEP(config->ss->step)) == ANADIG_PLL->PLL_528_SS))
        {
            /* no need to reconfigure the PLL if all the configuration is the same */
            if (0UL == (ANADIG_PLL->PLL_528_CTRL & ANADIG_PLL_PLL_528_CTRL_ENABLE_CLK_MASK))
            {
                ANADIG_PLL->PLL_528_CTRL |= ANADIG_PLL_PLL_528_CTRL_ENABLE_CLK_MASK;
            }

            if ((ANADIG_PLL->PLL_528_CTRL & ANADIG_PLL_PLL_528_CTRL_PLL_528_GATE_MASK) != 0UL)
            {
    ...
    if ((reg & (ANADIG_PLL_PLL_528_CTRL_POWERUP_MASK | ANADIG_PLL_PLL_528_CTRL_ENABLE_CLK_MASK)) != 0UL)
    {
        /* Power down the PLL. */
    ...
    {
        ANADIG_PLL->PLL_528_MFD = ANADIG_PLL_PLL_528_MFD_MFD(config->mfd);
        ANADIG_PLL->PLL_528_SS  = (ANADIG_PLL_PLL_528_SS_ENABLE_MASK | ANADIG_PLL_PLL_528_SS_STOP(config->ss->stop) |
                                  ANADIG_PLL_PLL_528_SS_STEP(config->ss->step));
    }
...
void CLOCK_DeinitSysPll2(void)
{
    ...
    ANADIG_PLL->PLL_528_SS &= ~ANADIG_PLL_PLL_528_SS_ENABLE_MASK;
}
...
{
    return ((ANADIG_PLL->PLL_528_PFD & (uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_DIV1_CLKGATE_MASK
                                           << (8UL * (uint8_t)pfd)) == 0U);
}
...
void CLOCK_InitPfd(clock_pll_t pll, clock_pfd_t pfd, uint8_t frac)
{
    ...
    regFracVal = ((*pfdCtrl) & ((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_FRAC_MASK << (8UL * (uint32_t)pfd))) >>
                 (8UL * (uint32_t)pfd);
    pfdGated =
        (bool)(((*pfdCtrl) & ((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_DIV1_CLKGATE_MASK << (8UL * (uint32_t)pfd))) >>
               (8UL >> (uint32_t)pfd));
    ...
    stable = *pfdCtrl & ((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_STABLE_MASK << (8UL * (uint32_t)pfd));
    *pfdCtrl |= ((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_DIV1_CLKGATE_MASK << (8UL * (uint32_t)pfd));

    /* all pfds support to be updated on-the-fly after corresponding PLL is stable */
    *pfdCtrl &= ~((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_FRAC_MASK << (8UL * (uint32_t)pfd));
    *pfdCtrl |= (ANADIG_PLL_PLL_528_PFD_PFD0_FRAC(frac) << (8UL * (uint32_t)pfd));

    *pfdUpdate ^= ((uint32_t)ANADIG_PLL_PLL_528_UPDATE_PFD0_UPDATE_MASK << (uint32_t)pfd);
    *pfdCtrl &= ~((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_DIV1_CLKGATE_MASK << (8UL * (uint32_t)pfd));
    /* Wait for stablizing */
    while (stable == (*pfdCtrl & ((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_STABLE_MASK << (8UL * (uint32_t)pfd))))
    {
    }
}
...
uint32_t CLOCK_GetPfdFreq(clock_pll_t pll, clock_pfd_t pfd)
{
    ...
    switch (pll)
    {
        case kCLOCK_PllSys2:
            frac =
                (ANADIG_PLL->PLL_528_PFD & ((uint32_t)ANADIG_PLL_PLL_528_PFD_PFD0_FRAC_MASK << (8UL * (uint32_t)pfd)));
            pllFreq = (uint32_t)PLL_SYS2_528_FREQ;
            break;
        case kCLOCK_PllSys3:
            frac =
                (ANADIG_PLL->PLL_480_PFD & ((uint32_t)ANADIG_PLL_PLL_480_PFD_PFD0_FRAC_MASK << (8UL * (uint32_t)pfd)));
...
void CLOCK_InitSysPll3(void)
{
    ...
    if (0UL != (ANADIG_PLL->PLL_480_CTRL & ANADIG_PLL_PLL_480_CTRL_POWERUP_MASK))
    {
        /* no need to reconfigure the PLL if all the configuration is the same */
        if (0UL == (ANADIG_PLL->PLL_480_CTRL & ANADIG_PLL_PLL_480_CTRL_ENABLE_CLK_MASK))
        {
            ANADIG_PLL->PLL_480_CTRL |= ANADIG_PLL_PLL_480_CTRL_ENABLE_CLK_MASK;
        }

        if (0UL != (ANADIG_PLL->PLL_480_CTRL & ANADIG_PLL_PLL_480_CTRL_PLL_480_GATE_MASK))
        {
     ...
     */
    reg                      = ANADIG_PLL_PLL_480_CTRL_PLL_REG_EN(1) | ANADIG_PLL_PLL_480_CTRL_PLL_480_GATE(1);
...
void CLOCK_DeinitSysPll3(void)
{
    ...
    ANADIG_PLL->PLL_480_CTRL |= ANADIG_PLL_PLL_480_CTRL_PLL_480_GATE_MASK;
    ANADIG_PLL->PLL_480_CTRL &= ~(ANADIG_PLL_PLL_480_CTRL_ENABLE_CLK_MASK | ANADIG_PLL_PLL_480_CTRL_POWERUP_MASK |
                                  ANADIG_PLL_PLL_480_CTRL_PLL_REG_EN_MASK);
}
...
{
    return ((ANADIG_PLL->PLL_480_PFD & (uint32_t)ANADIG_PLL_PLL_480_PFD_PFD0_DIV1_CLKGATE_MASK
                                           << (8UL * (uint8_t)pfd)) == 0U);
}
...
void CLOCK_SetPllBypass(clock_pll_t pll, bool bypass)
{
    switch (pll)
    {
        ...
        case kCLOCK_PllSys1:
            ...
            break;
        case kCLOCK_PllSys2:
            ANADIG_PLL->PLL_528_CTRL = bypass ? (ANADIG_PLL->PLL_528_CTRL | ANADIG_PLL_PLL_528_CTRL_BYPASS_MASK) :
                                                (ANADIG_PLL->PLL_528_CTRL & ~ANADIG_PLL_PLL_528_CTRL_BYPASS_MASK);
            break;
        case kCLOCK_PllSys3:
            ANADIG_PLL->PLL_480_CTRL = bypass ? (ANADIG_PLL->PLL_480_CTRL | ANADIG_PLL_PLL_480_CTRL_BYPASS_MASK) :
                                                (ANADIG_PLL->PLL_480_CTRL & ~ANADIG_PLL_PLL_480_CTRL_BYPASS_MASK);
            break;
