...
void SRC_SLICE_SoftwareControl(SRC_MIX_SLICE_Type *base, src_power_ctrl_step_t step)
{
    ...
    switch (step)
    {
        case kSRC_PDN_SentinelHandshake:
            base->SLICE_SW_CTRL |= SRC_MIX_SLICE_SLICE_SW_CTRL_SETL_HDSK_SOFT_MASK;
            while ((base->FUNC_STAT & SRC_MIX_SLICE_FUNC_STAT_SETL_HDSK_STAT_MASK) !=
                   SRC_MIX_SLICE_FUNC_STAT_SETL_HDSK_STAT_MASK)
            {
            }
            break;
        ...
        case kSRC_PUP_IsolationOff:
            ...
            break;
        case kSRC_PUP_SentinelHandshake:
            base->SLICE_SW_CTRL &= ~SRC_MIX_SLICE_SLICE_SW_CTRL_SETL_HDSK_SOFT_MASK;
            while ((base->FUNC_STAT & SRC_MIX_SLICE_FUNC_STAT_SETL_HDSK_STAT_MASK) != 0UL)
            {
            }
            break;
        default:
