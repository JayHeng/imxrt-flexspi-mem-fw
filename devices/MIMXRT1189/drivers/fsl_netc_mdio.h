...
{
    kNETC_InternalMdio, /*!< Bound handle to MAC port internal MDIO access. */
    kNETC_ExternalMdio, /*!< Bound handle to MAC port external MDIO access. */
} netc_mdio_type_t;
...
typedef struct _netc_mdio
{
    ...
    netc_hw_eth_port_idx_t port; /*!< MDIO port index. */
} netc_mdio_t;

typedef struct _netc_mdio_handle netc_mdio_handle_t;

/*! @brief MDIO PHY status callback function. */
typedef void (*netc_mdio_phy_status_callback_t)(netc_mdio_handle_t *handle,
                                                uint16_t intrLow2High,
                                                uint16_t intrHigh2Low);
...
struct _netc_mdio_handle
{
    ...
    netc_mdio_phy_status_callback_t callback; /*!< Interrupt callback for PHY status interrupt. */
};
...
/*!
 * @brief Setup the way to check PHY status automatically
 * If there's no special external interrupt to indicate certain PHY status such as link status, can use this mechanism
 * to get it without polling the PHY in software.
 *
 * @param handle MDIO handle.
 * @param config The configuration of the PHY status auto check.
 * @param callback Callback for the PHY status event.
 * @return status_t
 */
...
/*!
 * @brief Handle the PHY status interrupt.
 *
 * @param handle MDIO handle.
 */
...
/*!
 * @brief Clean the status transition interrupt flag(s).
 *
 * @param handle MDIO handle.
 * @param intrLow2High Clean the interrupt flag of a 0->1 transition on a corresponding bit.
 * @param intrHigh2Low Clean the interrupt flag of a 1->0 transition on a corresponding bit.
 */
void NETC_MDIOPhyStatusCleanInterrupt(netc_mdio_handle_t *handle, uint16_t intrLow2High, uint16_t intrHigh2Low);
