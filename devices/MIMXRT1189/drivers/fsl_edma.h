...
typedef struct _edma_transfer_config
{
    ...
    bool disableRequestAfterMajorLoopComplete; /*!< the channel's ERQ bit can be cleared after the major loop complete
                                                  automatically */
...
static inline void EDMA_SetChannelMux(EDMA_Type *base, uint32_t channel, dma_request_source_t channelRequestSource)
{
    ...
    EDMA_CHANNEL_BASE(base, channel)->CH_MUX = DMA_CH_MUX_SRC(channelRequestSource);
}
